from src.ui.shared_ui import clear_frames, create_right_frame, create_button, create_searchable_listbox, make_ho_radio_button
...
def load_new_game_page(parent_frame):
    ...
    create_button(right_frame, "Load Game", lambda: load_game_page(parent_frame), 0, 0, 1, 1)

    # create button to start new game
    create_button(right_frame, "New Game", lambda: new_game_page(parent_frame), 1, 0, 1, 1)
...
def load_game_page(parent_frame):
    ...
    create_button(right_frame, "Back", lambda: load_new_game_page(parent_frame), 1, 0, 1, 1)
...
def new_game_page(parent_frame):
    ...
    entry_var_tournament, selected_tournament_var = create_searchable_listbox(right_frame, "Tournament", tournaments,
                                                                              0, 0, 1, 2)
    ...
    entry_var_scouted, selected_scouted_var = create_searchable_listbox(right_frame, "Scouted Team", studied_teams,
                                                                        1, 0, 1, 2)
    ...
    entry_var_opponent, selected_opponent_var = create_searchable_listbox(right_frame, "Opponent", opponents,
                                                                          2, 0, 1, 2)

    # radio button for wind level
    wind_level_options = [wind.name for wind in WindSpeed]
    # make a radio button for the wind level
    wind_speed_var = make_ho_radio_button(right_frame, wind_level_options, "Wind Speed",
                                       3, 0, 1, 2)

    # create back button to go back to the start page
    create_button(right_frame, "Back", lambda: load_new_game_page(parent_frame), 4, 0, 1, 1)
    create_button(right_frame, "Next", lambda: print("hi"), 4, 1, 1, 1)
